...
for year in sorted(data_long['Year'].unique()):
    ...
    for country in frame_data[frame_data['Year'] == year].sort_values(by='Warheads', ascending=True)['Country'].unique():
        ...
        offset = 0
        while any(abs(final_warheads + offset - y) < 1000 for y in last_points):
            offset += 450
...
max_warheads = data_long['Warheads'].max() * 1.1

rounded_max = np.ceil(max_warheads / 10000) * 10000

tickvals = np.linspace(0, rounded_max, 6)

fig.update_layout(
    title=dict(text='Liczba głowic jądrowych w czasie według kraju', y=0.96, yref='container', yanchor='bottom'),
    xaxis=dict(title='', range=[data_long['Year'].min(), data_long['Year'].max()], showticklabels=False, showgrid=False),
    yaxis=dict(title='', range=[0, data_long['Warheads'].max() * 1.11], tickmode='array', tickvals=tickvals),
...
linear_animated_line_html_file_path = "Nuclear_Warheads_Line_Chart.html"
